...
const paramsId = Joi.object({
  idPago: Joi.number().integer().min(1).required().messages({
...
const getManyReqQuery = Joi.object({
  ...
  sortBy: Joi.string().trim().default('idPago').messages({
  ...
  monto_gte: Joi.number().min(0).messages({
    'number.base': 'Error de validación: El campo {#label} debe ser un número',
    'number.min':
      'Error de validación: El campo {#label} debe ser mayor o igual a {#limit}',
  }),
  monto_lte: Joi.number().min(0).messages({
    'number.base': 'Error de validación: El campo {#label} debe ser un número',
    'number.min':
      'Error de validación: El campo {#label} debe ser mayor o igual a {#limit}',
  }),
...
const patchReqBody = Joi.object({}).messages({
  'object.unknown': 'El parámetro {#label} no es permitido',
});
